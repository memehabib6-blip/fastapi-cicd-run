...
2. Setup the makefile with the following commands:
    ...
    <details>
    <summary markdown='span'>If you're on ARM architecture!</summary>

    If your machine is using an ARM based processor (like Apple's M-series), you'll need to rebuild to deploy.

    The reason: Google Cloud Run machines run on amd64 processors.

    How to know if you're on ARM? In your VM's terminal: `arch`.

    You need another build command with the option `--platform linux/amd64`.

    Remember this for future challenges!
